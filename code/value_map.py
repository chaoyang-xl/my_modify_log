...
class ValueMap(BaseMap):
    ...
    def update_map(
        ...
        if RECORDING:
            ...
            with open(JSON_PATH, "w") as f:
                json.dump(data, f)
    #4 13 修改sort_waypoints
    #   新增alpha = 1.0 beta = 0.5 作为系数 控制权重
    ...
    def sort_waypoints(
        ...
        alpha = 1.0
        # Disable IG term when obstacle_map is unavailable (e.g., Habitat V2 default)
        beta = 0.5 if self._obstacle_map is not None else 0.0

        # Use the maximum FOV cone area for information-gain normalization.
        ...
        for i, point in enumerate(waypoints):
            v = reduced_values[i]
            ig = self.compute_information_gain(point) if self._obstacle_map is not None else 0
    ...
    #4 13 新增compute_information_gain 
    #   计算某个 frontier 点的信息增益（information gain）
    #   信息增益 = 该点作为观测位置时，在传感器视野(FOV)内能看到的“未知区域”的像素数量
